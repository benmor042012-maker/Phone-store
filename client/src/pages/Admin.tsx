...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    if (content.data?.status === "unavailable") setNotice("מאגר התוכן אינו מחובר לאתר הזה כרגע.");
  }, [content.data, loaded]);
  ...
  const uploadImage = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (result.status === "too_large") { setNotice("התמונה גדולה מדי. הגבול הוא 2MB."); return; }
    setNotice(result.status === "bad_type" ? "ניתן להעלות JPG, PNG או WebP בלבד." : "העלאת התמונה נכשלה.");
  };
  ...
  const publishDraft = async () => {
    ...
    if (result.status === "too_large") { setNotice("התוכן גדול מדי לפרסום. לא בוצע שינוי."); return; }
    setNotice("הפרסום נכשל. לא בוצע שינוי בתוכן החנות.");
  };
