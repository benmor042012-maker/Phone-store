...
import {
  buildOrderMessage, buildProductShareText, CART_STORAGE_KEY, instantPaymentTargets, paymentMethods,
  readStoredList, SAVED_STORAGE_KEY, STORE, whatsappOrderUrl, writeStoredList,
  type PaymentMethodId,
} from "@/lib/storefrontState";
import { categoryPicture, onImageError, pictureSources, productPicture } from "@/lib/images";
...
import { SiteLogo, SiteNavLinks, siteNavLinks } from "@/components/storefront/SiteHeader";
import { AboutSection, ContactSection, FaqSection, HeroTitleBand, RepairsSection, SiteFooter } from "@/components/storefront/StaticSections";
import { HOME_SEO } from "@shared/pages";
import { productTitle } from "@shared/text";
import { useLocation } from "wouter";
import {
  Accessibility, ArrowLeft, ChevronLeft, ChevronRight, Eye, Heart, Menu, Minus, Search, Share2, ShoppingBag, SlidersHorizontal, Star, X,
} from "lucide-react";
 ...
 * A page of 24 keeps the first paint light on a phone; the rest is a button away.
 */
const PAGE_SIZE = 24;
...
export default function Home() {
  ...
  /**
   * Slide one is the clean one: the video and nothing over it. The slides that came before
   * follow it, so the arrows and the dots still move between them the way they always did.
   */
  const slideCount = heroSlides.length + 1;
  const hero = activeSlide > 0 ? heroSlides[activeSlide - 1] ?? heroSlides[0] : null;
  ...
  /**
   * The magnifier used to scroll to the catalog and stop there. On a phone the search field
   * lives inside the collapsed filter panel, so that left the shop with no way to search at
   * all: the field has to be opened and handed the cursor, not just scrolled past.
   */
  const openSearch = () => {
    setFiltersOpen(true);
    document.getElementById("catalog")?.scrollIntoView({ behavior: "smooth" });
    window.setTimeout(() => searchRef.current?.focus(), 340);
  };
  ...
  useEffect(() => {
    ...
    if (selectedProduct) {
      ...
      applyPageSeo({
        title: productTitle(selectedProduct.name, ` | ${STORE.name} נתניה`),
  ...
  return (
    <div className={`store-page ${highContrast ? "high-contrast" : ""}`} dir="rtl" style={largeText ? { fontSize: "112%" } : undefined}>
      ...
      <header className="store-header">
        <SiteLogo href="#top" onError={onImageError} />
        <SiteNavLinks links={siteNavLinks(locale, true)} open={menuOpen} onNavigate={() => setMenuOpen(false)} />
      ...
      <main>
      {/* The first slide carries the video alone; from the second on, the slides are the
          ones the shop already had, copy and all. The page's heading sits under the hero,
          so the clean slide stays clean and the heading never changes with the slides. */}
      <section className={hero ? "hero-original" : "hero-original hero-clean"} id="top">
        ...
        {hero && <><div className="hero-haze" /><div aria-hidden="true" style={{ position: "absolute", zIndex: -1, left: "6%", top: "-42%", width: "22%", height: "172%", transform: "rotate(20deg)", border: "1px solid rgba(213,169,69,.22)", pointerEvents: "none" }} /><div aria-hidden="true" style={{ position: "absolute", zIndex: -1, right: "6%", top: "-42%", width: "22%", height: "172%", transform: "rotate(-20deg)", border: "1px solid rgba(213,169,69,.22)", pointerEvents: "none" }} /></>}
        {hero
          ? <div className="hero-inner"><p className="hero-label">חנות סלולרי בנתניה · PHONE STORE</p><p className="hero-slide-title">{hero[0]}<em>{hero[1]}</em></p><p>{hero[2]}</p><div className="hero-buttons"><a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noreferrer" className="btn-gold">דברו עם אלי ב־WhatsApp <ArrowLeft size={17} /></a><a href="#catalog" className="btn-outline">לצפייה במלאי</a></div></div>
          : null}
        {hero && <div className="live-stock"><span /><strong>{products.length.toLocaleString("he-IL")} פריטים</strong><small>{sourceQuery.data?.status === "live" ? "מלאי מסונכרן · משלוח מהיר" : "זמינים עכשיו · משלוח מהיר"}</small></div>}
        <div className="hero-controls"><button onClick={() => setActiveSlide((activeSlide + slideCount - 1) % slideCount)} aria-label="השקופית הקודמת"><ChevronRight size={19} /></button>{Array.from({ length: slideCount }, (_, index) => <button key={index} className={index === activeSlide ? "dot active" : "dot"} onClick={() => setActiveSlide(index)} aria-label={`מעבר לשקופית ${index + 1}`} />)}<button onClick={() => setActiveSlide((activeSlide + 1) % slideCount)} aria-label="השקופית הבאה"><ChevronLeft size={19} /></button></div>
      </section>
      <HeroTitleBand />
      ...
      <SiteFooter whatsapp={whatsappNumber} phoneHref={STORE.phoneHref} />
